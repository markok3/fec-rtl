...
const CardPreview: React.FC<CardPreviewProps> = ({
  ...
  return (
    <div
      className={`w-[380px] text-white rounded-xl bg-blue`}
      style={{
    ...
    >
      <div className="flex flex-col w-full  ">
        <div className="flex flex-row w-full p-4 justify-between items-center">
          ...
          <span>
            {intl.formatMessage({ id: "cardformat.preview.points" })}: 0
          </span>
        </div>

        {cardFormat && cardFormat.image ? (
          <img src={cardFormat.image} alt="Card" className="w-auto h-[180px]" />
        ) : (
          <Image
            src={Background}
            alt="background"
        ...
        <div className="p-4">
          <h2 className="text-xl font-semibold">
            {intl.formatMessage({ id: "cardformat.preview.cardHolder" })}
          </h2>
          <span>{cardHolder ? cardHolder : "Card Holder Name"}</span>
        </div>
